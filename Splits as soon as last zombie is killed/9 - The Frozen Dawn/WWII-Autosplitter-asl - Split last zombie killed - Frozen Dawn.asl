state("s2_mp64_ship")
{
    ...
    int round_is_completed : 0x273BD74;
}
...
reset
{
    if(current.round_counter == 0 && old.round_counter != 0 || current.map_name.Equals("mp_hub_zombies_0"))
    {
        vars.timer_start = 0;
...
split
{
    ...
    else if(current.round_is_completed > vars.prev_round_completed && current.round_is_completed <= 1065353216)
    {
